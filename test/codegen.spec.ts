...
describe("generateWorkflowCode", () => {
  it("should map ComfyUI node defs into typed WorkflowBuilder methods", () => {
    ...
    expect(code).toContain("KSampler(inputs: KSamplerInputs): KSamplerOutputs & { __id: string }");
  ...
  it("should sanitize special node names and append suffixes for duplicates", () => {
    ...
    expect(code).toContain('/** Node: "CR-Multi ControlNet Stack" */');
    expect(code).toContain("CRMultiControlNetStack(inputs: CRMultiControlNetStackInputs): CRMultiControlNetStackOutputs & { __id: string }");
    expect(code).toContain(
      "CRMultiControlNetStack2(inputs: CRMultiControlNetStack2Inputs): CRMultiControlNetStack2Outputs & { __id: string }"
    );
  ...
  it("should return only __id for output nodes without outputs", () => {
    ...
    expect(code).toContain("SaveImage(inputs: SaveImageInputs): { __id: string }");
    expect(code).toContain("return { __id: id };");
  });
});
