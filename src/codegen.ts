...
function isImageInput(def: unknown): boolean {
...
function buildResultLines(nodeDef: NodeDef, outputLines: Array<{ key: string; refTypeParam: string }>): string[] {
  const lines: string[] = [];
  if (outputLines.length > 0) {
    for (const output of outputLines) {
      lines.push(`  ${quoteKey(output.key)}?: ComfyOutputValue<${output.refTypeParam}>;`);
    }
    return lines;
  }

  const required = nodeDef.input?.required ?? {};
  const hasImageInput = Object.values(required).some((value) => isImageInput(value));
  if (nodeDef.output_node && hasImageInput) {
    lines.push('  images?: ComfyOutputValue<"IMAGE">;');
  }
...
export function generateWorkflowCode(nodeDefs: NodeDefsResponse, options?: GenerateWorkflowCodeOptions): string {
  ...
  for (const key of keys) {
    ...
    const resultInterfaceName = `${safeName}Result`;
    ...
    const resultLines = buildResultLines(nodeDef, outputLines);
    interfaces.push(`export interface ${resultInterfaceName} extends ComfyNodeOutput {`);
    interfaces.push(...resultLines);
    interfaces.push(`}`);
    interfaces.push("");
    ...
    const methodHeader = outputLines.length
      ? `  ${safeName}(inputs: ${inputInterfaceName}${inputLines.length === 0 ? " = {}" : ""}): ${outputInterfaceName} & { __id: OutputNodeId<${resultInterfaceName}>; inputs: ${inputPathsInterfaceName} } {`
      : `  ${safeName}(inputs: ${inputInterfaceName}${inputLines.length === 0 ? " = {}" : ""}): { __id: OutputNodeId<${resultInterfaceName}>; inputs: ${inputPathsInterfaceName} } {`;
    ...
    if (outputLines.length > 0) {
      ...
      methods.push(`      __id: id as OutputNodeId<${resultInterfaceName}>,`);
      methods.push("      inputs: inputPaths");
      methods.push("    };");
    } else {
      methods.push(`    return { __id: id as OutputNodeId<${resultInterfaceName}>, inputs: inputPaths };`);
    }
    methods.push("  }");
    methods.push("");
  }
  ...
  return [
    ...
    `import { WorkflowBuilder as BaseWorkflowBuilder, NodeRef, OutputNodeId, ComfyOutputValue, ComfyNodeOutput } from ${JSON.stringify(sdkImportPath)};`,
    "",
    ...interfaces,
